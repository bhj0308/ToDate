...
import { useConversation, useConversationSocket } from "../hooks/useStructured";

export function ConversationScreen({ matchId }: { matchId: string }) {
  const { user } = useAuth();
  const conversation = useConversation(matchId);
  const { status, sendMessage } = useConversationSocket(matchId);
  ...
  return (
    ...
    >
      ...
      />
      {status !== "open" && <StatusMessage variant="loading" message="Reconnecting…" />}
      <View style={styles.composer}>
        <View style={styles.input}>
          <TextField label="" placeholder="Message" value={draft} onChangeText={setDraft} />
        </View>
        <PrimaryButton title="Send" onPress={handleSend} disabled={!draft.trim() || status !== "open"} />
      </View>
    </KeyboardAvoidingView>
  );
}
